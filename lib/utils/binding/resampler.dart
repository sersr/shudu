...
class PointerEventResampler {
  ...
  Offset _positionAt(Duration sampleTime) {
    ...
    final nextTimeStamp = _next?.timeStamp ?? Duration.zero;
    final lastTimeStamp = _last?.timeStamp ?? Duration.zero;
    ...
    if (nextTimeStamp > sampleTime && nextTimeStamp > lastTimeStamp) {
      ...
      final lastX = _last?.position.dx ?? 0.0;
      final lastY = _last?.position.dy ?? 0.0;
  ...
  void sample(
    ...
    if (_queuedEvents.isNotEmpty) {
      final now = _queuedEvents.last.timeStamp;
      _lastTime ??= now;
      Log.i(
          'event: ${(now.inMicroseconds - _lastTime!.inMicroseconds) / 1000} ms | sample');
