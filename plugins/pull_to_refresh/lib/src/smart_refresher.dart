...
class RefreshController {
  ...
  late ValueNotifier<RefreshStatus> headerMode;

  /// footer status mode controll
  late ValueNotifier<LoadStatus> footerMode;
  ...
  @Deprecated('advice set ScrollController to child,use it directly will cause bug when call jumpTo() and animateTo()')
  ...
  Future<void>? requestRefresh(
      {bool needMove: true, Duration duration: const Duration(milliseconds: 500), Curve curve: Curves.linear}) {
    ...
    if (needMove) SmartRefresher.ofState(position!.context.storageContext).setCanDrag(false);
    WidgetsBinding.instance!.addPostFrameCallback((_) {
      if (needMove) {
  ...
  Future<void>? requestLoading(
      {bool needMove: true, Duration duration: const Duration(milliseconds: 300), Curve curve: Curves.linear}) {
